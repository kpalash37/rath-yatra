            ...
            <div className="flex flex-col space-y-4">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-gray-700 hover:text-orange-600 transition-colors font-medium px-2 py-1 rounded hover:bg-orange-50"
                  onClick={() => setIsOpen(false)}
                >
                  {item.name}
                </Link>
              ))}
              <Button className="bg-gradient-to-r from-orange-600 to-red-500 hover:from-red-500 hover:to-orange-600 w-fit">
                <Heart className="mr-2 h-4 w-4" />
                Donate Now
              </Button>
            </div>
